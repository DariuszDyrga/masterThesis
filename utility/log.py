...
class Log:
    def __init__(self, log_each: int, initial_epoch=-1, log_name='results'):
        self.best_accuracy = 0.0
        self.log_each = log_each
        self.epoch = initial_epoch
        self.log_name = log_name + '.txt'
    ...
    def flush(self) -> None:
        if self.is_train:
            ...
            #epoch train
            print(
                f"\r┃{self.epoch:12d}  ┃{loss:12.4f}  │{accuracy:10.2f} %  ┃{self.learning_rate:12.3e}  │{self._time():>12}  ┃",
                end="",
                flush=True,
            )
            with open(self.log_name, 'a') as file:
                file.write(f"\r{self.epoch},{loss},{accuracy},{self.learning_rate},{self._time()},")
        else:
            #valid
            loss = self.epoch_state["loss"] / self.epoch_state["steps"]
            accuracy = self.epoch_state["accuracy"] / self.epoch_state["steps"]
            ...
            with open(self.log_name, 'a') as file:
                file.write(f"{loss},{100*accuracy}")
    ...
    def _time(self) -> str:
        ...
        return f"{time_seconds // 60:02}:{time_seconds % 60:02}"
    ...
    def _print_header(self) -> None:
        ...
        print(f"┠──────────────╂──────────────┼──────────────╂──────────────┼──────────────╂──────────────┼──────────────┨")
        f = open(self.log_name, "x")
        f.close()
        with open(self.log_name, 'a') as file:
            file.write(f"epoch,loss_train,accuracy_train,lr,elapsed,loss_valid,accuracy_valid\n")